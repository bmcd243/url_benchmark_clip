...
def force_pose(physics, pose_name):
    with physics.reset_context():
        ...
        if pose_name == "Standing":
            ...
            physics.named.data.qpos['rooty'] = np.random.uniform(1.2, 1.9)
        ...
        elif pose_name == "Crouching":
            # Low squat — visually similar height to mid-stride but different semantics
            physics.named.data.qpos['rootz'] = -0.35
            physics.named.data.qpos['rooty'] = np.random.uniform(-0.1, 0.1)
            physics.named.data.qpos['right_hip'] = np.random.uniform(0.7, 1.0)
            physics.named.data.qpos['right_knee'] = np.random.uniform(-1.4, -1.0)
            physics.named.data.qpos['left_hip'] = np.random.uniform(0.7, 1.0)
            physics.named.data.qpos['left_knee'] = np.random.uniform(-1.4, -1.0)
            ...
            physics.named.data.qpos['rooty'] = np.random.uniform(2.5, 3.5) # Upside down
            ...
            physics.named.data.qpos['rooty'] = 0.5   # lean forward
            physics.named.data.qpos['right_hip'] = 0.1
            physics.named.data.qpos['left_hip'] = 0.1
            physics.named.data.qpos['right_knee'] = -0.1
            physics.named.data.qpos['left_knee'] = -0.1

        elif pose_name == "Leaning-Back":
            physics.named.data.qpos['rootz'] = -0.05
            physics.named.data.qpos['rooty'] = -0.5  # lean backward
            physics.named.data.qpos['right_hip'] = -0.1
            physics.named.data.qpos['left_hip'] = -0.1
            physics.named.data.qpos['right_knee'] = -0.1
            physics.named.data.qpos['left_knee'] = -0.1
            ...
            physics.named.data.qpos['rootz'] = -0.05
            physics.named.data.qpos['rooty'] = np.random.uniform(-0.1, 0.1)
            physics.named.data.qpos['right_hip'] = 1.2   # raised leg
            physics.named.data.qpos['right_knee'] = -1.0
            physics.named.data.qpos['left_hip'] = 0.0    # standing leg
            physics.named.data.qpos['left_knee'] = -0.05

        elif pose_name == "Landing":
            physics.named.data.qpos['rootz'] = -0.3
            physics.named.data.qpos['rooty'] = np.random.uniform(-0.1, 0.1)
            physics.named.data.qpos['right_hip'] = 0.8
            physics.named.data.qpos['right_knee'] = -1.3
            physics.named.data.qpos['left_hip'] = 0.8
            physics.named.data.qpos['left_knee'] = -1.3

        elif pose_name == "Stumbling":
            physics.named.data.qpos['rootz'] = -0.6
            physics.named.data.qpos['rooty'] = np.random.uniform(0.8, 1.2)
            physics.named.data.qpos['right_hip'] = np.random.uniform(-0.5, 0.5)
            physics.named.data.qpos['left_hip'] = np.random.uniform(-0.5, 0.5)
            physics.named.data.qpos['right_knee'] = np.random.uniform(-1.0, 0)
            physics.named.data.qpos['left_knee'] = np.random.uniform(-1.0, 0)
