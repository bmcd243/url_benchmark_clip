...
def get_model_and_assets():
    ...
    xml = resources.GetResource(os.path.join(task_dir, 'texturedwalker.xml'))
    if isinstance(xml, bytes):
        xml = xml.decode('utf-8')

    assets = common.ASSETS.copy()

    # Filenames must match exactly what texturedwalker.xml references
...
def stand(time_limit=_DEFAULT_TIME_LIMIT,
    ...
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = PlanarWalker(move_speed=0,
                        forward=True,
                        flip=False,
                        random=random)
    environment_kwargs = environment_kwargs or {}
...
@SUITE.add('benchmarking')
def walk(time_limit=_DEFAULT_TIME_LIMIT,
         random=None,
         environment_kwargs=None):
    """Returns the Walk task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = PlanarWalker(move_speed=_WALK_SPEED,
                        forward=True,
                        flip=False,
                        random=random)
    environment_kwargs = environment_kwargs or {}
...
class PlanarWalker(base.Task):
    ...
    def __init__(self, move_speed, forward=True, flip=False, random=None):
